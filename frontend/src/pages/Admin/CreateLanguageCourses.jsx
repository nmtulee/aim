...
import { useCreateLanguageCourseMutation } from '../../redux/api/languageCourseApiSlice.js';
import {
...
const CreateLanguageCourses = () => {
  const navigate = useNavigate();
  const [createLanguageCourse, { isLoading }] =
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Filter out empty levels and trim values
      ...
      await createLanguageCourse(languageCourseData).unwrap();

      setMessage('Language course created successfully!');
      setMessageType('success');
      ...
      // Navigate to language courses list after a short delay
      setTimeout(() => {
        navigate(-1);
      }, 2000);
    } catch (error) {
      setMessage(
        error?.data?.message ||
          'Failed to create language course. Please try again.'
      );
      setMessageType('error');
    }
  };
    ...
    <div className='container my-10 mx-auto px-4 py-8 max-w-4xl'>
      ...
      <div className='mb-8'>
        ...
        <h1 className='text-3xl font-bold text-gray-900 mb-2'>
          Create Language Course
        </h1>
        <p className='text-gray-600'>
          Fill in the details below to create a new language course
        </p>
      </div>
...
export default CreateLanguageCourses;
