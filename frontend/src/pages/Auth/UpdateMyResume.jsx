...
const UpdateMyResume = () => {
  ...
  const [updateResume] = useUpdateMyResumeMutation();

  // Redirect if no resume exists
  useEffect(() => {
    if (!myResume) {
  ...
  // Prefill form if resume exists
  useEffect(() => {
    if (myResume) {
      setFullName(myResume.fullName || '');
      setJobTitle(myResume.jobTitle || '');
      setPhoto(myResume.photo || null);
      setPdf(myResume.file || null);
      setCategory(myResume.category || '');
    }
  }, [myResume]);
  ...
  const isChanged =
    ...
    photo !== (myResume?.photo || null) ||
    pdf !== (myResume?.file || null);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!fullName || !jobTitle || !category || !photo || !pdf) {
    ...
    try {
      ...
      navigate('/myResume'); // Navigate to resume page after update
    } catch (err) {
